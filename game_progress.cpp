...
void Game::paintEvent(QPaintEvent *event)
{
  ...
  if(gameOver){
    ...
    if(life<=50 && !hp->isEaten())
        painter.drawImage(hp->getRect(), hp->getImage());
    for(int i=0; i<3; i++){
        if(!ice[i]->isEaten())
...
{
    //gives hp regeneration item when health low
    if(life<=50 && !hp_spawn){
        hp = new HP(250,250);
        hp->setEaten(FALSE);
        hp_spawn = TRUE;
    ...
    if(hp_spawn && !hp->isEaten()){
        if((player->getRect()).intersects(hp->getRect())){
            life+=25;
            hp->setEaten(TRUE);
        }
    }
 ...
 /*if((player->getRect()).intersects(penta->getRect()) && !penta_shield){
     life -=33;
     penta_shield = TRUE;
 }
